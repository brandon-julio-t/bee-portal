...
@section('body')
    <div x-data="{ showModal: false, action: '', userId: '', userName: '', userEmail: '' }">
        <div class="container">
            <form action="{{ route('admin.manage-students') }}" id="filter-form" class="mb-4">
                ...
                <label>
                    <input type="checkbox" name="exclude_deleted" class="form-check" @if (request()->has('exclude_deleted')) checked @endif @change="() => { document.querySelector('#filter-form').submit(); }">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody>
                                    @foreach ($students as $student)
                                        <tr
                                            class="{{ $student->trashed() ? ($loop->odd ? 'bg-red-50' : 'bg-red-100') . ' hover:bg-red-200' : ($loop->odd ? 'bg-white' : 'bg-gray-50') . ' hover:bg-gray-100' }}">
                                                ...
                                                <div x-data="{ showDeleteDialog: false }" class="inline-block relative">
                                                    @if ($student->trashed())
                                                        <div x-cloak x-transition x-show="showDeleteDialog"
                                                            ...
                                                            <form
                                                                action="{{ route('admin.manage-users.restore', $student) }}"
                                                    ...
                                                    @else
                                                        <div x-cloak x-transition x-show="showDeleteDialog"
                                                            class="absolute bottom-0 right-0 z-10">
                                                            <form
                                                                action="{{ route('admin.manage-users.delete', $student) }}"
                                                                method="POST" class="card bg-white">
                                                                @csrf
                                                                @method('DELETE')
